...
class Timestamp<T> implements Comparable<Timestamp<?>>{     // zaso u comparable timestamp ima argument type od wildcard?
...
class Scheduler<T> extends Timestamp<T>{
    ...
    public Scheduler(){
        timeStampList = new ArrayList<Timestamp<T>>(0);
    }
    ...
    public boolean remove(Timestamp<T> t){      // dali moze taa funkcija da se naprave so ternary operator?
        boolean toDo = timeStampList.contains(t);
        if(toDo){
    ...
    public Timestamp<T> next(){
        ...
        Comparator<Timestamp<T>> timestampComparator = Comparator.comparing(t -> t.time.compareTo(now));
...
public class SchedulerTest {//SchedulerTest
